...
// Review Pipeline
// ============================================
const handleReview = async (octokit, owner, repo, prNumber, fetchExistingBotComments = false) => {
    ...
    const extractResult = (r, name, fallback) => {
    ...
    // Orchestrator로 결과 병합
    const result = await runOrchestrator(orchProvider, config.agents.orchestrator.model, orchPrompt, { diff, qualityIssues, performanceIssues: perfIssues, securityIssues, existingComments }, config.options.max_comments_per_review, config.agents.orchestrator.temperature, config.agents.orchestrator.max_tokens);
    // 리뷰 게시
...
// Resolver
// ============================================
const handleResolve = async (octokit, graphqlFn, owner, repo, prNumber) => {
    const config = loadConfig();
    const aiReviewDir = getAiReviewDir();
...
// Responder
// ============================================
const handleRespond = async (octokit, owner, repo, prNumber, comment) => {
    const config = loadConfig();
    const aiReviewDir = getAiReviewDir();
